...
```{r compare-site-usage-pRPS3}
polyA_site_position = 69

RPS3_cumulative_polyA_reads <- full_length_quantseq_plasmids_cumulative_polyA_reads %>%
  filter(promoter == "pRPS3")

example_chosen_poly_a_site<- ggplot(RPS3_cumulative_polyA_reads) +
  geom_vline(xintercept = polyA_site_position) +
  geom_line(aes(x = read_end, y = rel_cumulative_counts, linetype = str_detect(sample, "A"), group = sample, colour = mod)) +
  lims(x = c(0,100)) 

RPS3_pre_polyA_reads_ratio <- RPS3_cumulative_polyA_reads %>%
  group_by(sample, mod) %>%
  do(calc_polyA_site_usage(., polyA_site_position))

example_poly_a_site_usage_ratio <- ggplot(RPS3_pre_polyA_reads_ratio) +
  geom_point(aes(x = mod, y = pre_polyA_site_count_ratio))

save(example_poly_a_site_usage_ratio,  example_chosen_poly_a_site, file = here("poly_a_site_usage_exmaple.RData"))
```